  ...
  getComments().then((querySnapshot) => {
   ...
   `;
    ...
    querySnapshot.forEach((doc) => {
      ...
      const closed = document.getElementById(`windowContent-${doc.id}`);
      window.onclick = (event) => {   
        
        if (event.target == closed) {      //target busca el evento click 
          console.log('closed'); 
          closed.classList.toggle('windowContent');
          //closed.style.display = "none";
          //document.getElementById(`windowContent-${doc.id}`).classList.toggle('windowContent');
        }
