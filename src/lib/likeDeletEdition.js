  ...
  db.collection("comments")
    ...
    .then((doc) => {
      ...
      if (postLike.like.includes(emailData)) {
        for (let i = 0; i < postLike.like.length; i += 1) {
          // recorre el array de like y agrega 1
          if (postLike.like[i] === emailData) {
            //verficando que el like del usuario no exista ya
            postLike.like.splice(i, 1); //splice elimina elemento del array
            db.collection("comments").doc(id).update({
              //agrega a la coleccion el nuevo like
              like: postLike.like,
            });
          }
        }
